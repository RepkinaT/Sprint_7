...
class TestCourierCreation:
    ...
    @allure.description('Создаем запись курьера передавая логин, пароль и имя. Должны получить код: 201 и'
                        ' текст: "ok":true')
    def test_successful_courier_creation(self, get_courier_data_and_delete_courier):
        data = get_courier_data_and_delete_courier
        response = CourierCreationHelper.courier_creation_request(data)
    ...
    def test_of_impossibility_of_creating_two_identical_couriers(self,
        ...
        data = get_courier_data_and_delete_courier
        CourierCreationHelper.courier_creation_request(data)
        response = CourierCreationHelper.courier_creation_request(data)
    ...
    @allure.description('Пытаемся создать запись без указания имени. Должны получить код: 201 и '
                        'текст: {"ok":true}')
    def test_for_creating_a_courier_without_name_data(self, get_courier_data_and_delete_courier):
        data = get_courier_data_and_delete_courier
        del data["firstname"]
        response = CourierCreationHelper.courier_creation_request(data)
        AssertHelper.assert_func(response, 201, Data.status_code.get(201)[0])
